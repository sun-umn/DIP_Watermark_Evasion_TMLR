...
def main(args):
    ...
    pure_noise_bgr_normed = float_to_uint8((pure_noise_normed_float - value_min) / (value_max - value_min))
    ...
    for num_iter in range(total_iters):
        ...
        if num_iter % show_every == 0:
            ...
            img_recon_uint8_np = img_recon_int_np.astype(np.int8)
            if save_interm:
            ...
            # mse and psnr w.r.t pure noise
            pure_noise_float = pure_noise_int_np.astype(np.float32) / 255.
            mse_noise = np.mean((residual_recon - pure_noise_float)**2)
            psnr_noise = compute_psnr(
                (residual_recon*255).astype(np.int16),
                pure_noise_int_np,
                data_range=(np.amax(pure_noise_int_np)-np.amin(pure_noise_int_np))
            )
            # mse 
            ...
            mse_noise_log.append(mse_noise)
            psnr_noise_log.append(psnr_noise)
    ...
    res = {
        ...
        "mse_noise": mse_noise_log,
        "psnr_noise": psnr_noise_log,
    ...
    l1 = ax.plot(iter_data, res["mse_clean"], label="recon v.s clean", alpha=0.5, lw=2)
    l2 = ax.plot(iter_data, res["mse_noise"], label="(recon - clean) v.s. pure noise", alpha=0.5, lw=2, ls="dashed")
    l3 = ax.plot(iter_data, res["mse_noisy_img"], label="recon v.s. input (noisy image)", alpha=0.5, lw=2)
    ...
    l2 = ax.plot(iter_data, res["psnr_noise"], label="(recon - clean) v.s. pure noise")
    ...
    print("  Clean - [{}] | Noise - [{}]".format(mse_clean, mse_noise))
